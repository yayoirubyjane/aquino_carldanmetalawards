...
@section('content')
    ...
    <form action="{{ route('orders.update', $order->OrderID) }}" method="POST" class="max-w-4xl">
        @csrf
        @method('PUT')
        ...
        <div class="bg-gray-50 p-4 rounded mb-6 border border-gray-300">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="OrderDate" value="{{ old('OrderDate', $order->OrderDate) }}" required class="w-full border border-gray-300 p-2 rounded">
                    @error('OrderDate')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="DeliveryDate" value="{{ old('DeliveryDate', $order->DeliveryDate) }}" required class="w-full border border-gray-300 p-2 rounded">
                    @error('DeliveryDate')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="OrderStatus" required class="w-full border border-gray-300 p-2 rounded bg-white">
                        <option value="Pending" {{ old('OrderStatus', $order->OrderStatus) == 'Pending' ? 'selected' : '' }}>Pending</option>
                        <option value="In Production" {{ old('OrderStatus', $order->OrderStatus) == 'In Production' ? 'selected' : '' }}>In Production</option>
                        <option value="Completed" {{ old('OrderStatus', $order->OrderStatus) == 'Completed' ? 'selected' : '' }}>Completed</option>
                    </select>
                    @error('OrderStatus')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
        ...
        <div class="mt-6">
            <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold px-4 py-2 rounded">Update Order</button>
    ...
    <script>
        ...
        const existingProducts = @json($order->productOrders);
        ...
        existingProducts.forEach(po => {
            addProductRow(po.ProductID, po.Quantity, po.Price);
        });

        // Initialize with one empty row if no products
