...
@section('content')
    @php
        $editingProducts = old('products', $existingOrderProducts);
    @endphp

    <div class="page-card">
        <h2 class="mb-6 text-2xl font-bold text-slate-900">Edit Order PO-{{ str_pad($order->OrderID, 4, '0', STR_PAD_LEFT) }}</h2>
        ...
        <form action="{{ route('orders.update', $order->OrderID) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-5">
                ...
                <div>
                    ...
                    <select name="EmployeeID" class="page-select" required>
                        @foreach ($employees as $employee)
                            <option value="{{ $employee->EmployeeID }}" @selected(old('EmployeeID', $order->EmployeeID) == $employee->EmployeeID)>{{ $employee->full_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="OrderStatus" class="page-select" required>
                        @foreach (['Pending', 'In Production', 'Completed'] as $status)
                            <option value="{{ $status }}" @selected(old('OrderStatus', $order->OrderStatus) === $status)>{{ $status }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="OrderDate" value="{{ old('OrderDate', optional($order->OrderDate)->format('Y-m-d')) }}" class="page-input" required>
                </div>
            ...
            <div class="rounded-xl border border-sky-100 bg-sky-50/50 p-4">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-slate-500">Items already in production cannot be removed from the production list automatically.</p>
                    </div>
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                <div class="page-card border border-sky-100 bg-sky-50/40">
                    <h3 class="mb-3 text-lg font-semibold text-slate-900">Payment Summary</h3>
                    <p class="text-sm text-slate-500">Paid: PHP {{ number_format($order->amount_paid, 2) }}</p>
                    <p class="text-sm text-slate-500">Balance: PHP {{ number_format($order->balance_due, 2) }}</p>
                </div>

                <div class="page-card border border-sky-100 bg-sky-50/40 text-right">
                    <p class="text-sm text-slate-500">Total amount</p>
                    <p id="total-amount" class="text-3xl font-bold text-sky-700">PHP {{ number_format($order->total_amount, 2) }}</p>
                </div>
            </div>

            <div class="flex gap-3">
                <button type="submit" class="page-button-primary">Update Order</button>
    ...
    <script>
        ...
        const oldProducts = @json($editingProducts);
