...
@section('content')
    <div class="page-card max-w-4xl">
        <h2 class="mb-6 text-2xl font-bold text-slate-900">Edit Product</h2>
        ...
        <form action="{{ route('products.update', $product->ProductID) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="ProductName" value="{{ old('ProductName', $product->ProductName) }}" class="page-input" required>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Product Type</label>
                    <input type="text" name="ProductType" value="{{ old('ProductType', $product->ProductType) }}" class="page-input" required>
                </div>
            </div>

            <div class="max-w-sm">
                <label class="mb-2 block text-sm font-semibold text-slate-700">Selling Price</label>
                <input type="number" step="0.01" min="0" name="Price" value="{{ old('Price', $product->Price) }}" class="page-input" required>
            </div>
            ...
            <div class="flex gap-3">
                <button type="submit" class="page-button-primary">Update Product</button>
